...
export function SentimentAnalysisCard({ stats = DEFAULT_STATS, className }: SentimentAnalysisCardProps) {
  ...
  const highIntensityPercentage = stats.total > 0 ? ((stats.highIntensity / stats.total) * 100).toFixed(1) : '0';
  const mediumIntensityPercentage = stats.total > 0 ? ((stats.mediumIntensity / stats.total) * 100).toFixed(1) : '0';
  const lowIntensityPercentage = stats.total > 0 ? ((stats.lowIntensity / stats.total) * 100).toFixed(1) : '0';
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <Badge variant="outline" className="text-gray-600 border-gray-300">
              {stats.neutral} ({neutralPercentage}%)
            </Badge>
          </div>
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <Badge variant="outline" className="text-red-600 border-red-300">
              {stats.negative} ({negativePercentage}%)
            </Badge>
          </div>
        ...
        <div className="space-y-3 pt-3 border-t">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center p-3 rounded-lg bg-red-50 border border-red-200">
              <div className="text-2xl font-bold text-red-600">{stats.highIntensity}</div>
            ...
            <div className="text-center p-3 rounded-lg bg-yellow-50 border border-yellow-200">
              <div className="text-2xl font-bold text-yellow-600">{stats.mediumIntensity}</div>
            ...
            <div className="text-center p-3 rounded-lg bg-green-50 border border-green-200">
              <div className="text-2xl font-bold text-green-600">{stats.lowIntensity}</div>

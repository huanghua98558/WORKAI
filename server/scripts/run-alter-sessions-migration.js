...
async function runMigration() {
  ...
  try {
    const sqlPath = path.join(__dirname, '../database/migrations/016_alter_sessions_table.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');

    console.log('开始执行迁移: 016_alter_sessions_table.sql');
    console.log('SQL路径:', sqlPath);

    await pool.query(sql);

    console.log('✅ sessions表更新成功！');
  } catch (error) {

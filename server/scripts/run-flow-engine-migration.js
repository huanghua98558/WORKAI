...
async function runMigration() {
  ...
  try {
    const sqlPath = path.join(__dirname, '../database/migrations/009_create_flow_engine_tables.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');

    console.log('开始执行迁移: 009_create_flow_engine_tables.sql');
    console.log('SQL路径:', sqlPath);

    await pool.query(sql);

    console.log('✅ 流程引擎相关表创建成功！');
  } catch (error) {

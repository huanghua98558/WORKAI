...
const worktoolCallbackRoutes = async function (fastify, options) {
  ...
  const { callbackHistory, flowDefinitions, videoChannelUsers } = require('../database/schema');
   ...
   * 视频号转化系统 - 用户添加好友回调
   * 
   * 当用户添加转化客服机器人好友时，WorkTool会发送回调通知
   * 该接口用于：
   * 1. 接收用户添加好友的回调通知
   * 2. 检查是否是转化客服机器人（config.role = 'conversion'）
   * 3. 自动创建用户记录到 video_channel_users 表
   * 
   * 请求参数：
   * - userId: 用户ID
   * - userName: 用户昵称
   ...
   * - timestamp: 时间戳
   */
  fastify.post('/video-channel-user', {
    preHandler: []
  }, async (request, reply) => {
    const startTime = Date.now();
    const requestId = generateRequestId();
    ...
    try {
      ...
      // 验证 callbackData
      if (!callbackData.userId || !callbackData.userName) {
        console.error('❌ 缺少必要参数：userId 或 userName');
        return reply.status(400).send(errorResponse(400, '缺少必要参数：userId 或 userName'));
      }
      ...
      // 检查是否是转化客服机器人
      const robotConfig = robot.config || {};
      if (robotConfig.role !== 'conversion') {
        console.log(`⚠️  机器人不是转化客服机器人，跳过处理: ${robot.name} (role: ${robotConfig.role})`);
        return reply.send(successResponse({}, 'success'));
      }

      console.log(`✅ 确认是转化客服机器人: ${robot.name}`);

      // 记录审计日志
      await auditLogger.log('video_channel_user_callback', 'worktool', {
        requestId,
        robotId,
        callbackData
      });
      ...
      const existingUser = await db.select()
        ...
        .where(eq(videoChannelUsers.userId, callbackData.userId))
        .limit(1);

      if (existingUser && existingUser.length > 0) {
        console.log(`⚠️  用户已存在，更新最后活跃时间: ${callbackData.userName} (${callbackData.userId})`);
        // 更新最后活跃时间
      ...
      await db.insert(videoChannelUsers).values(newUser);

      console.log(`✅ 成功创建用户记录: ${callbackData.userName} (${callbackData.userId})`, {
        userId: newUser.id,
        robotId: robot.robotId,
        robotName: robot.name,
      ...
      // 记录回调历史
      const responseTime = Date.now() - startTime;
      await recordCallbackHistory(robotId, 'video_channel_user', requestId, 0, '', { 
        responseTime,
        userId: callbackData.userId,
        userName: callbackData.userName
      });
      ...
      const responseTime = Date.now() - startTime;
      await recordCallbackHistory(robotId, 'video_channel_user', requestId, 500, error.message, { responseTime });
